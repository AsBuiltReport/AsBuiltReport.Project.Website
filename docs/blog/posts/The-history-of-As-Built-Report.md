...
Soon after VMworld, we began work on redesigning the core architecture of AsBuiltReport to make it more modular to support different report modules. This eventually evolved into the AsBuiltReport framework and the [AsBuiltReport.Core module](https://www.powershellgallery.com/packages/AsBuiltReport.Core/){:target="_blank"}. With the core framework now in place, I worked on refactoring the VMware vSphere script and developing some other report modules.
